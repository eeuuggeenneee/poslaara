            ...
            <tbody>
                @foreach ($lineItems as $item)
                    <tr>
                        <td>{{ $item['name'] }}</td>
                        <td>
                            <button class="btn btn-sm btn-danger" wire:click="subtractQuantity({{ $item['product_id'] }})">-</button>
                            {{ $item['quantity'] }}
                            <button class="btn btn-sm btn-success" wire:click="addQuantity({{ $item['product_id'] }})">+</button>
                        </td>
                        <td>${{ $item['price'] * $item['quantity'] }}</td>
                        <td><button class="btn btn-sm btn-primary" wire:click="removeProduct({{ $item['product_id'] }})">Remove</button></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
