<div class="mt-5">
        ...
        <ul class="list-group mt-2">
            @foreach ($searchResults as $result)
                @php
                    $itemKey = array_search($result->id, array_column($lineItems, 'product_id'));
                    $cartQuantity = $itemKey === false ? 0 : $lineItems[$itemKey]['quantity'];
                @endphp

                @if ($cartQuantity < $result->quantity)
                    <li class="list-group-item list-group-item-action" wire:click="addProduct({{ $result->id }})">
                        {{ $result->name }} - ${{ $result->price }}
                    </li>
                @else
                    <li class="list-group-item list-group-item-action text-muted">
                        {{ $result->name }} - ${{ $result->price }} (Out of stock)
                    </li>
                @endif
            @endforeach
                ...
                @foreach ($lineItems as $index => $item)
                    <tr>
                        ...
                        <td><button class="btn btn-sm btn-primary"
                                wire:click="removeProduct({{ $item['product_id'] }})">Remove</button></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <input type="number" class="form-control" wire:model="cashGiven"
                                wire:keyup="calculateChange" placeholder="Enter cash amount given by customer" />
                            <h5 class="mt-3 text-center">Change: ${{ number_format($change, 2) }}</h5>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary"
                                wire:click="$set('showCompleteSaleModal', false)">Close</button>
                            <button type="button" class="btn btn-primary" wire:click="completeSale">Complete
                                Sale</button>
                        </div>
                    </div>
                </div>
